...
| Version | Key updates |
|---|---|
| v0.8.30 | Added a script fidelity and traceability rule: dialogue/action lines get stable IDs, a coverage ledger at the end of Phase 2 must reach 100% coverage (no lost lines) before Phase 3; Phase 4/5 prompts must open with a SOURCE ANCHOR block plus MUST HAVE/MUST NOT HAVE lists, and generation review must fill a mandatory "what did the output add that wasn't asked for" field. First-frame drift hard-blocks; video drift is a soft gate unless it violates an explicit exclusion. Fixes two real failures: a shot table silently dropping script dialogue, and a first frame adding an unrequested prop that survived into the final video. |
