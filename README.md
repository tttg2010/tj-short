...
- Seedance2 正式首帧/宫格不降级成整张漫画：默认使用摄影画面 + 局部脸部 `face_pencil`，备选才是“模糊主图 + 五官特写”两张图
...
4. 正式首帧/宫格默认采用 `face_pencil`：只把脸部做彩铅/手绘化，身体、服装、产品、动作和场景保持摄影质感。
5. 提示词中定义主体：`将图片1中的面部特征、图片2中的服装造型定义为林夏`，后续持续使用同一角色名。
6. 备选 `blur_feature` 两张图：主图模糊脸部保留构图，另给五官特写图补足虚拟角色特征。
