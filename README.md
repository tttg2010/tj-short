...
Codex + salpx 中转站的短剧带货 Skill：在 Codex 里自动完成产品分析、脚本、分镜、首帧、提示词、字幕计划和交付检查，再用 salpx / omni_flash 执行图生视频。
...
产品分析 -> 三个 brief -> 产品证明圣经 -> 高冲突剧本 -> 12 镜头分镜 -> 9:16 首帧 -> `salpx / omni_flash` 固定 10 秒提示词 -> 字幕与投放切片计划。

这里的分工很明确：

- **Codex 是生产大脑**：负责判断、写作、落文件、提示词、manifest、字幕、复盘和隐私检查。
- **salpx 中转站是视频执行层**：负责把 Codex 生成的干净 9:16 首帧和剧本锁定 prompt 送进 `omni_flash`，产出固定 10 秒图生视频素材。
...
- salpx 中转站负责把首帧和 prompt 变成 `omni_flash` 视频素材
...
| salpx / omni_flash | 真正执行图生视频 | Codex 负责判断、脚本、提示词、manifest 和交付检查 |
...
flowchart TD
  ...
  I --> J["Codex 编译 salpx / omni_flash 固定 10 秒 prompt"]
  J --> K["salpx 中转站执行图生视频"]
  K --> L["返回 10 秒 raw clips"]
